...
追踪目标函数的调用耗时，记录入口到返回的时间区间，用于测量函数执行时间。
...
1. attach 到目标进程后，在目标函数**入口**注入 `INT3`（`0xCC`）断点。
2. 入口断点触发时，记录时间戳，并在**返回地址**处注入第二个 INT3。
3. 返回断点触发时计算耗时，恢复原始指令，输出一条 `TraceNode`。
4. 重新安装入口断点，等待下一次调用。

**当前限制：** 每次只追踪目标函数本身（单层），`TraceNode.children` 当前始终为空，子调用树追踪功能规划中。每次断点触发会使目标线程短暂停顿（微秒级）。
...
当前每次 trace 只输出目标函数本身一行；子调用展开功能规划中。
